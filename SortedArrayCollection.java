...
public class SortedArrayCollection<T> implements CollectionInterface<T>  
{
  ...
  //returns the smallest element in the collection
  {
   if(isEmpty())
      ...
      return elements[0];//due to sorted array code we can just use the element at index 0 since it will be smallest
  }
  public int greater(T element)
  //returns the number of elements that are greater than T element 
  {
    if(isEmpty())
       return 0;
       find(element);
       int count = location;
    if(found)
    // will count one for every element greater than it till the end of the collection
    {
         while(elements[count].equals(element))
         count++;
    }
  ...
  //combine two collections 
  {
     ...
     SortedArrayCollection<T> both = new SortedArrayCollection<>(size1 + size2);
     
     while(first < size1 && second < size2)
     {
     if(((Comparable)(elements[first])).compareTo(other.elements[second]) < 0)
     both.add(elements[first++]);
     else
     both.add(other.elements[second++]);
     }
     while(first < size1)
     both.add(elements[first++]);
     while(second < size2)
